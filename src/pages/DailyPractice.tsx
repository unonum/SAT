...
import { Dumbbell, Timer, Zap, Layers, History, Rocket, ArrowRight, X, Loader2 } from 'lucide-react';
...
function buildFallback(def: ModeDef, topics: TopicId[] | undefined, attempts: import('@/lib/types').Attempt[]): Question[] {
  ...
  if (def.mode === 'topic' && topics?.length) {
    ...
    return (pool.length >= def.count ? pool.sort(() => Math.random() - 0.5).slice(0, def.count)
...
export default function DailyPractice() {
      ...
      if (remoteEnabled && user?.email) {
        try {
          questions = await createNovelTestSession({
            ...
            topics,
            attempts,
            fallbackQuestions: buildFallback(def, topics, attempts),
          });
        } catch {
          questions = buildFallback(def, topics, attempts);
        }
      } else {
        questions = buildFallback(def, topics, attempts);
      }
      if (!questions.length) questions = buildFallback(def, topics, attempts);
  ...
  return (
    <div className="space-y-6">
      <SectionTitle
        title="Practice"
        subtitle="Novel questions generated from your uploaded study materials using AI — no two sessions repeat."
      />
      {error && <p className="text-sm text-danger">{error}</p>}

      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {MODES.map((m, i) => {
          const loading = loadingMode === m.mode;
          return (
            <motion.button
              key={m.mode}
              ...
              onClick={() => !isLoading && void start(m)}
              disabled={isLoading}
              className="card p-6 text-left hover:shadow-glow hover:-translate-y-1 hover:border-brand-400/40 transition-all duration-300 group disabled:opacity-60 disabled:cursor-not-allowed disabled:hover:translate-y-0"
            >
              <div className={`mb-4 grid h-12 w-12 place-items-center rounded-2xl bg-gradient-to-br ${m.grad} text-white shadow-soft`}>
                {loading ? <Loader2 size={22} className="animate-spin" /> : m.icon}
              </div>
              ...
              <p className="text-sm text-muted leading-relaxed">
                {loading ? 'Generating novel questions…' : m.desc}
              </p>
              <div className="mt-4 flex items-center justify-between text-sm">
                ...
                <span className="flex items-center gap-1 font-bold text-brand-600 dark:text-brand-300 group-hover:gap-2 transition-all">
                  {loading ? 'Building…' : <>Start <ArrowRight size={14} /></>}
                </span>
              </div>
            </motion.button>
          );
        ...
        <motion.button
          ...
          disabled={isLoading}
          className="card p-6 text-left hover:shadow-glow hover:-translate-y-1 transition-all duration-300 disabled:opacity-60"
        >
            ...
            <div className="grid gap-2.5 sm:grid-cols-2 lg:grid-cols-4">
              {TOPICS.map((t) => (
                <motion.button
                  ...
                  whileTap={{ scale: 0.97 }}
                  disabled={isLoading}
                  onClick={() => !isLoading && void start(
                    { mode: 'topic', title: `Topic: ${t.name}`, desc: '', icon: null, count: 10, grad: '' },
                    [t.id]
                  )}
                  className="rounded-2xl border border-[rgb(var(--border))] p-4 text-left hover:border-brand-400 hover:bg-brand-500/5 hover:shadow-glow transition-all disabled:opacity-60"
                >
